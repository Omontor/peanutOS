<div class="m-3">
    @can('project_story_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.project-stories.create') }}">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-projectProjectStories">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $projectStory->id }}">
                                ...
                                <td>
                                    @can('project_story_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.project-stories.show', $projectStory->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('project_story_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.project-stories.edit', $projectStory->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('project_story_delete')
                                        <form action="{{ route('admin.project-stories.destroy', $projectStory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('project_story_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.project-stories.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-projectProjectStories:not(.ajaxTable)').DataTable({ buttons: dtButtons })
