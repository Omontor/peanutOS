...
@can('project_documentation_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.project-documentations.create') }}">
                        ...
                        <tr data-entry-id="{{ $projectDocumentation->id }}">
                            ...
                            <td>
                                @can('project_documentation_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.project-documentations.show', $projectDocumentation->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('project_documentation_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.project-documentations.edit', $projectDocumentation->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('project_documentation_delete')
                                    <form action="{{ route('admin.project-documentations.destroy', $projectDocumentation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('project_documentation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.project-documentations.massDestroy') }}",
