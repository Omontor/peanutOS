...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="asignee_id">{{ trans('cruds.task.fields.asignee') }}</label>
                <select class="form-control select2 {{ $errors->has('asignee') ? 'is-invalid' : '' }}" name="asignee_id" id="asignee_id" required>
                    @foreach($asignees as $id => $entry)
                        <option value="{{ $id }}" {{ (old('asignee_id') ? old('asignee_id') : $task->asignee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('asignee'))
                    <span class="text-danger">{{ $errors->first('asignee') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.asignee_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="documentation">{{ trans('cruds.task.fields.documentation') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.documentation_helper') }}</span>
            </div>
            <div class="form-group">
...
@if(isset($task) && $task->image)
      var file = {!! json_encode($task->image) !!}
...
<script>
    ...
    success: function (file, response) {
      $('form').find('input[name="documentation"]').remove()
    ...
    init: function () {
@if(isset($task) && $task->documentation)
      var file = {!! json_encode($task->documentation) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
