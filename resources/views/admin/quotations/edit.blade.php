    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.quotations.update", [$quotation->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $quotation->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assets') ? 'is-invalid' : '' }}" name="assets[]" id="assets" multiple required>
                    @foreach($assets as $id => $assets)
                        <option value="{{ $id }}" {{ (in_array($id, old('assets', [])) || $quotation->assets->contains($id)) ? 'selected' : '' }}>{{ $assets }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" name="total" id="total" value="{{ old('total', $quotation->total) }}" step="0.01" required>
                @if($errors->has('total'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('clauses') ? 'is-invalid' : '' }}" name="clauses[]" id="clauses" multiple>
                    @foreach($clauses as $id => $clauses)
                        <option value="{{ $id }}" {{ (in_array($id, old('clauses', [])) || $quotation->clauses->contains($id)) ? 'selected' : '' }}>{{ $clauses }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('from') ? 'is-invalid' : '' }}" type="text" name="from" id="from" value="{{ old('from', $quotation->from) }}">
                @if($errors->has('from'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('to') ? 'is-invalid' : '' }}" type="text" name="to" id="to" value="{{ old('to', $quotation->to) }}">
                @if($errors->has('to'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('validity') ? 'is-invalid' : '' }}" type="number" name="validity" id="validity" value="{{ old('validity', $quotation->validity) }}" step="1">
