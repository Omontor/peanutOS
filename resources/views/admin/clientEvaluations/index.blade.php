...
@can('client_evaluation_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.client-evaluations.create') }}">
                        ...
                        <tr data-entry-id="{{ $clientEvaluation->id }}">
                            ...
                            <td>
                                @can('client_evaluation_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.client-evaluations.show', $clientEvaluation->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('client_evaluation_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.client-evaluations.edit', $clientEvaluation->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('client_evaluation_delete')
                                    <form action="{{ route('admin.client-evaluations.destroy', $clientEvaluation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('client_evaluation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.client-evaluations.massDestroy') }}",
