    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contracts.update", [$contract->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('contract_date') ? 'is-invalid' : '' }}" type="text" name="contract_date" id="contract_date" value="{{ old('contract_date', $contract->contract_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('contract_deadline') ? 'is-invalid' : '' }}" type="text" name="contract_deadline" id="contract_deadline" value="{{ old('contract_deadline', $contract->contract_deadline) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('from') ? 'is-invalid' : '' }}" name="from_id" id="from_id" required>
                    @foreach($froms as $id => $entry)
                        <option value="{{ $id }}" {{ (old('from_id') ? old('from_id') : $contract->from->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('from'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('static_clauses') ? 'is-invalid' : '' }}" name="static_clauses[]" id="static_clauses" multiple>
                    @foreach($static_clauses as $id => $static_clause)
                        <option value="{{ $id }}" {{ (in_array($id, old('static_clauses', [])) || $contract->static_clauses->contains($id)) ? 'selected' : '' }}>{{ $static_clause }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dynamic_clauses') ? 'is-invalid' : '' }}" name="dynamic_clauses[]" id="dynamic_clauses" multiple>
                    @foreach($dynamic_clauses as $id => $dynamic_clause)
                        <option value="{{ $id }}" {{ (in_array($id, old('dynamic_clauses', [])) || $contract->dynamic_clauses->contains($id)) ? 'selected' : '' }}>{{ $dynamic_clause }}</option>
                    @endforeach
                </select>
