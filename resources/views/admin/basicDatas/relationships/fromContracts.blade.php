    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-fromContracts">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        @foreach($contracts as $key => $contract)
                            <tr data-entry-id="{{ $contract->id }}">
                                ...
                                <td>
                                    @foreach($contract->static_clauses as $key => $item)
                                        <span class="badge badge-info">{{ $item->title }}</span>
                                    @endforeach
                                </td>
                                <td>
                                    @foreach($contract->dynamic_clauses as $key => $item)
                                        <span class="badge badge-info">{{ $item->title }}</span>
                                    @endforeach
                                </td>
                                <td>
...
@endcan
  ...
  let table = $('.datatable-fromContracts:not(.ajaxTable)').DataTable({ buttons: dtButtons })
