        ...
        <div class="col-md-12">
            @can('approval_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.approvals.create') }}">
                            ...
                            <tbody>
                                @foreach($approvals as $key => $approval)
                                    <tr data-entry-id="{{ $approval->id }}">
                                        ...
                                        <td>
                                            @can('approval_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.approvals.show', $approval->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('approval_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.approvals.edit', $approval->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('approval_delete')
                                                <form action="{{ route('frontend.approvals.destroy', $approval->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('approval_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.approvals.massDestroy') }}",
