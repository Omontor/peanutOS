                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.appointments.update", [$appointment->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $appointment->title) }}">
                            @if($errors->has('title'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="project_id" id="project_id" required>
                                @foreach($projects as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('project_id') ? old('project_id') : $appointment->project->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="invitees[]" id="invitees" multiple required>
                                @foreach($invitees as $id => $invitee)
                                    <option value="{{ $id }}" {{ (in_array($id, old('invitees', [])) || $appointment->invitees->contains($id)) ? 'selected' : '' }}>{{ $invitee }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control datetime" type="text" name="from" id="from" value="{{ old('from', $appointment->from) }}" required>
                            @if($errors->has('from'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('from') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control datetime" type="text" name="to" id="to" value="{{ old('to', $appointment->to) }}" required>
                            @if($errors->has('to'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('to') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="platform" id="platform" required>
                                ...
                                @foreach(App\Models\Appointment::PLATFORM_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('platform', $appointment->platform) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
