        ...
        <div class="col-md-12">
            @can('project_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.project-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $projectCategory->id }}">
                                        ...
                                        <td>
                                            @can('project_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.project-categories.show', $projectCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('project_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.project-categories.edit', $projectCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('project_category_delete')
                                                <form action="{{ route('frontend.project-categories.destroy', $projectCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('project_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.project-categories.massDestroy') }}",
