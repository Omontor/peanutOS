        ...
        <div class="col-md-12">
            @can('venue_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.venues.create') }}">
                            ...
                            <tbody>
                                @foreach($venues as $key => $venue)
                                    <tr data-entry-id="{{ $venue->id }}">
                                        ...
                                        <td>
                                            @can('venue_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.venues.show', $venue->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('venue_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.venues.edit', $venue->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('venue_delete')
                                                <form action="{{ route('frontend.venues.destroy', $venue->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('venue_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.venues.massDestroy') }}",
