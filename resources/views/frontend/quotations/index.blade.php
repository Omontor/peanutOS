        ...
        <div class="col-md-12">
            @can('quotation_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.quotations.create') }}">
                            ...
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="search">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach($rental_clauses as $key => $item)
                            ...
                            <tbody>
                                @foreach($quotations as $key => $quotation)
                                    <tr data-entry-id="{{ $quotation->id }}">
                                        ...
                                        <td>
                                            @foreach($quotation->assets as $key => $item)
                                                <span>{{ $item->name }}</span>
                                            @endforeach
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @foreach($quotation->clauses as $key => $item)
                                        ...
                                        <td>
                                            @can('quotation_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.quotations.show', $quotation->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('quotation_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.quotations.edit', $quotation->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('quotation_delete')
                                                <form action="{{ route('frontend.quotations.destroy', $quotation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('quotation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.quotations.massDestroy') }}",
